...
from google.cloud import storage # <--- 新增 GCS 導入
...
# --- 應用程式設定 ---
APP_VERSION = "v2.2.1 (GCS Attachment Test)" # <--- 更新版本號，標記 GCS 測試
...
GCS_BUCKET_NAME = "procurement-attachments-bucket"  # <--- GCS 儲存桶名稱
GCS_ATTACHMENT_FOLDER = "attachments"               # <--- 附件存放目錄
...
# --- UI 邏輯處理函式 (V2.2.1 重新引入 GCS/附件邏輯) ---
...
def set_preview_url(gcs_uri):
    ...
    if gcs_uri.startswith("gs://"):
        ...
        # else: # 錯誤處理已在 get_signed_attachment_url 內完成
...
def handle_add_new_quote(latest_arrival_date, uploaded_file): # <--- 重新加入 uploaded_file 參數
    """處理新增報價的邏輯 (V2.2.1 支援附件)"""
    ...
    new_row = {
        ...
        '標記刪除': False,
        '附件URL': attachment_uri # <--- 重新加入 '附件URL'
    }
...
def initialize_session_state():
    ...
    initial_values = {
        ...
        'delete_count': 0,
        'show_preview_modal': False, # <--- 重新加入：控制預覽 Modal
        'preview_url': "",           # <--- 重新加入：預覽的 URL
    }
    ...
    # *** 側邊欄 UI 邏輯 *** with st.sidebar:
        ...
        with st.expander("➕ 新增報價", expanded=False):
            ...
            if st.button("新增資料", key="btn_add_quote"):
                handle_add_new_quote(latest_arrival_date, uploaded_file) # <--- 重新加入 uploaded_file 參數
        ...
        with st.expander(label=f"專案：{proj_name} (點擊展開)", expanded=False):
            ...
            for item_name, item_data in proj_data.groupby('專案項目'):
                ...
                edited_df_value = st.data_editor(
                    editable_df[['ID', '選取', '供應商', '單價', '數量', '總價', '交期顯示', '狀態', '附件URL', '標記刪除']], # <--- 重新加入 '附件URL'
                    column_config={
                        ...
                        "附件URL": st.column_config.TextColumn("附件URL", help="GCS URI 或外部連結", disabled=False, width="medium"), # <--- 重新加入 "附件URL" column config
                    },
                    key=editor_key,
                    hide_index=True,
